...
const TOOL_CONFIGS = {
  ...
  'eslint.config.js': 'eslint', 'eslint.config.mjs': 'eslint', 'eslint.config.cjs': 'eslint', 'eslint.config.ts': 'eslint',
  '.prettierrc': 'prettier', '.prettierrc.json': 'prettier', '.prettierrc.yml': 'prettier', '.prettierrc.yaml': 'prettier', 'prettier.config.js': 'prettier', 'prettier.config.mjs': 'prettier', 'prettier.config.cjs': 'prettier',
  'lefthook.yml': 'lefthook', 'lefthook.yaml': 'lefthook',
  'turbo.json': 'turbo', 'nx.json': 'nx', 'pnpm-workspace.yaml': 'pnpm-workspace',
...
const TOOL_DEPENDENCIES = {
  ...
  'husky': 'husky', 'lefthook': 'lefthook', 'turbo': 'turbo', 'nx': 'nx',
  'wrangler': 'cloudflare', '@railway/cli': 'railway',
  'react-native': 'react-native', 'gsap': 'gsap',
  'react-email': 'react-email', '@react-email/components': 'react-email',
};
...
export async function scanProject(projectPath, { deep = false } = {}) {
  ...
  // Check common test directories at root level
  if (
    await dirExists(join(projectPath, 'tests'))
    || await dirExists(join(projectPath, 'test'))
    || await dirExists(join(projectPath, 'spec'))
    || await dirExists(join(projectPath, '__tests__'))
  ) {
  ...
  if (pkg) {
    ...
    for (const [dep, tool] of Object.entries(TOOL_DEPENDENCIES)) {
      if (allDeps[dep]) profile.techStack.add(tool);
    }
    ...
    if (pkg.workspaces || await exists(join(projectPath, 'pnpm-workspace.yaml'))
        ...
        || await exists(join(projectPath, 'turbo.json')) || allDeps.nx || allDeps.turbo) {
  ...
  // Project tooling
  for (const [file, tool] of Object.entries(TOOL_CONFIGS)) {
    if (await exists(join(projectPath, file))) profile.techStack.add(tool);
  }
  if (await dirExists(join(projectPath, '.husky'))) profile.techStack.add('husky');

  if (await exists(join(projectPath, 'pnpm-workspace.yaml')) || await exists(join(projectPath, 'turbo.json'))
      || await exists(join(projectPath, 'nx.json'))) {
    profile.isMonorepo = true;
  }
  ...
  if (deep) {
    // Check test files exist across multiple languages
    if (!profile.hasTests) {
      try {
        const entries = await readdir(projectPath, { recursive: true });
        const testFiles = entries.filter(e =>
          /\.(test|spec)\.(js|ts|jsx|tsx|mjs|py|rs|go|rb|php)$/i.test(e)
          || /(test_|tests\/|_test|Test\.)/i.test(e)
