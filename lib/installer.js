...
/**
 * Format a single skill choice for checkbox prompt.
 */
function formatChoiceLabel(recommendation) {
  ...
  const reason = chalk.dim(truncate(recommendation.reason, 35));
 ...
 * @param {{ auto?: boolean, scope?: 'global'|'project'|null, projectPath?: string, runCommand?: Function }} options
 */
export async function installSkills(recommendations, {
  auto = false, scope = null, projectPath = '.', runCommand = execFileAsync,
} = {}) {
  ...
  if (auto) {
    ...
    // Filter out any potential non-object entries just in case
