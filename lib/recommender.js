...
// Repositories known to be broken, private, or unresponsive during npx skills add
const BROKEN_REPOS = [
...
export async function getRecommendations(profile, {
  ...
  // Run all queries
  const allSkills = new Map(); // key (skillId or normalized name) → recommendation
  const cachedQueries = [];
  ...
  for (const { query, tech, priority } of queryEntries) {
    ...
    for (const skill of results) {
      ...
      // Use skillId as primary key, fallback to name for deduplication
  ...
  // Deduplicate by skill name (keep the highest installs/priority)
...
function isRelevantSkill({ skillId, name }, techStack) {
  // Check broken repos
  if (BROKEN_REPOS.some(repo => skillId.startsWith(repo))) return false;
